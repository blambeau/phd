...
The equation above is the well known one from model checking; it simply requires the system not to exhibit traces that violate the safety property. 
...
In fact, $\mathcal{L}^{-}(G)$ and $\mathcal{L}^{+}(G)$ are \emph{complement} of each other. That is, $\mathcal{L}^{+}(G) = \Sigma^{*} \setminus \mathcal{L}^{-}(G)$. Interestingly, the complement of a regular language is a regular language~\cite{Hopcroft:1979}, which implies that both can be modeled with a standard automaton. In contrast, the complement of a prefix-closed language is not prefix-closed, as we have illustrated on the example.  
