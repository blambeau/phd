...
We will use the common notations from formal language theory. Let $\Sigma$ denote a finite alphabet. $a$, $b$, $c$ denote elements of $\Sigma$, sometimes called \emph{words}. $u$, $v$, $w$ denote \emph{strings} over $\Sigma$ and $\lambda$ denotes the empty string. A string $u$ is a prefix of $v$ if there exists a string $w$ such that $uw = v$. A language $L$ is any subset of the set $\Sigma^*$ of strings over $\Sigma$. 

In essence, words here correspond to event labels in our background material from Chapter~\ref{chapter:framework} whereas strings correspond to traces. We also recall the definition of finite automaton below; for recall, a LTS is a special case of finite automaton with all accepting states. Section~\ref{subsection:inductive-lts-synthesis-reduction} describes the reduction of LTS synthesis to DFA induction in more details. 
...
$\mathcal{L}^+(Sc)$ and $\mathcal{L}^-(Sc)$ form valid samples for grammar induction. They can be respectively used as positive and negative input samples of the RPNI algorithm. As $\mathcal{L}^+(Sc)$ is prefix-closed the regular language learned by RPNI will be prefix-closed as well. Therefore the resulting DFA is a LTS; in other words, it contains only accepting states. 

This \emph{System} LTS covers all positive traces from $\mathcal{L}^+(Sc)$ and rejects all negative ones from $\mathcal{L}^-(Sc)$. In other words, it covers all positive scenarios and rejects all negative scenarios from $Sc$. Therefore, the System LTS and the scenario collection $Sc$ are consistent.
