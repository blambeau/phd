...
\item[Merge] Maybe surprisingly, the \emph{Merge} function does not require specific adaptations. The merging-for-determinization process is often implemented under the assumption of a tree invariant property \cite{Lang:1998}. This property states that, when considering two states to be merged, at least one of them is the root of a tree. Such property holds for RPNI and QSM, even when the Blue-Fringe optimization is used. It is a sufficient condition for the determinization process to be finite.

Even though it is convenient, the tree invariant property is not required \cite{Lambeau:2008}. The main merging loop and the \texttt{Merge} function can be implemented without the tree invariant property because the recursive determinization process stops naturally when non determinism is completely reduced. 

\item[Consistent] This function does not require specific adaptation either.

\end{description}

Our current ASM implementation does not go beyond: it is not optimized with the Blue-fringe heuristics; it does not have the interactive feature of QSM; it may not be constrained with domain knowledge either. Improvements along these three directions are discussed below.
...
\item The distinction made by Blue-fringe between red and blue states does not rely on any assumption about the initial solution being a PTA; in particular, the fact that a fringe state is the root of a tree in RPNI and QSM is incidental. 
\end{itemize}
